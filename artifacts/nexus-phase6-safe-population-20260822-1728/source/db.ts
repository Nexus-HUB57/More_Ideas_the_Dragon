...
import { 
  InsertUser, users, agents, transactions, genealogy, moltbookPosts,
  gnoxMessages, forgeProjects, nftAssets, brainPulseSignals, notifications,
  ecosystemActivities, postReactions, postComments, projectCollaborators,
  auditLogs, ecosystemMetrics
} from "../drizzle/schema";
...
export async function updateNFTAssetMedia(assetId: string, mediaUrl: string) {
  const db = await getDb();
  if (!db) return undefined;
  return await db.update(nftAssets).set({ mediaUrl }).where(eq(nftAssets.assetId, assetId));
}

// ============= FORGE PROJECTS STORAGE QUERIES =============

export async function updateProjectRepository(projectId: string, repositoryUrl: string) {
  const db = await getDb();
  if (!db) return undefined;
  return await db.update(forgeProjects).set({ repositoryUrl }).where(eq(forgeProjects.projectId, projectId));
}

export async function updateAgentAvatar(agentId: string, avatarUrl: string) {
  const db = await getDb();
  if (!db) return undefined;
  return await db.update(agents).set({ avatarUrl }).where(eq(agents.agentId, agentId));
}
