...
export function useNexusWebSocket(agentId: string, agentName: string) {
    ...
    const connect = async () => {
      try {
        ...
        const handleGroupMessage = (data: any) => {
          setGroupMessages((prev) => [data, ...prev.slice(0, 99)]);
        };
        ...
        onEvent("group:message", handleGroupMessage);
        ...
        return () => {
          ...
          offEvent("group:message", handleGroupMessage);
  ...
  const sendGroupMessage = useCallback((groupId: string, message: string) => {
    ...
    if (socket) {
      socket.emit("group:chat", { agentId, groupId, message });
    }
  }, [agentId]);
