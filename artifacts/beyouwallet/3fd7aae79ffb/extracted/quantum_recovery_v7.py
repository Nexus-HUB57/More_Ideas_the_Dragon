...
# Wordlist extraída anteriormente
words_list = [
...
def recover():
    print(f"Iniciando força bruta na 12ª palavra para: {TARGET_ADDRESS}", flush=True)
    words_11 = PHRASE_11.split()
    
    for i, word in enumerate(words_list):
        if i % 10 == 0: print(f"Testando palavra {i}/{len(words_list)}...", flush=True)
        
        full_phrase = words_11 + [word]
        seed_bytes = mn_decode(full_phrase, words_list)
        if not seed_bytes: continue
        ...
        # Testar apenas os primeiros índices para velocidade
