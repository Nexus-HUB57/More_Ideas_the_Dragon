...
def get_sequence(mpk, for_change, n):
    return string_to_number(Hash(("%d:%d:" % (n, for_change)).encode('utf-8') + mpk))
...
def hash_160(public_key):
    try:
        ...
        # Fallback para quando ripemd160 não está disponível no hashlib (OpenSSL 3.0+)
        from Crypto.Hash import RIPEMD160
        h = RIPEMD160.new()
        h.update(sha256(public_key))
        return h.digest()
...
def recover():
    ...
    # Derivar MPK (Master Public Key) para usar no get_sequence
    curve = SECP256k1
    master_private_key = ecdsa.SigningKey.from_secret_exponent(secexp, curve=SECP256k1)
    mpk = master_private_key.get_verifying_key().to_string()
    print(f"MPK derivado: {mpk.hex()}")
    ...
    for for_change in [0, 1]:
        ...
        for n in range(2000): # Geralmente os endereços estão nos primeiros índices
    ...
    print("\n--- FALHA ---")
    print("Endereço não encontrado nos primeiros 2000 índices.")
