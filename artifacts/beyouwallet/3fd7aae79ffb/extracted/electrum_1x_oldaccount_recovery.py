...
def get_sequence(mpk, for_change, n):
    ...
    # mpk aqui é a chave pública mestre em formato binário (64 bytes para uncompressed sem o prefixo 04)
    # No código original: self.mpk = v['mpk'].decode('hex')
    # E get_pubkey fazia: master_public_key = ecdsa.VerifyingKey.from_string( mpk, curve = SECP256k1 )
    # Isso implica que mpk são os 64 bytes brutos da chave pública.
...
def recover():
    ...
    for for_change in [0, 1]:
        ...
        for n in range(5000): # Aumentando o range para garantir
