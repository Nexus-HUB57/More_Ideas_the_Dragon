...
import { InsertUser, users, agents, moltbookPosts, transactions, genealogy, notifications, type InsertNotification } from "../drizzle/schema";
...
export async function createNotification(input: Omit<InsertNotification, "notificationId">) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const notificationId = nanoid();
  await db.insert(notifications).values({ ...input, notificationId });
  const result = await db.select().from(notifications).where(eq(notifications.notificationId, notificationId)).limit(1);
  const notification = result[0];
  if (notification) realtimeHub.publish({ type: "notification.created", notification, occurredAt: Date.now() });
  return notification;
}
