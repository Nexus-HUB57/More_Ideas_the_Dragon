...
class SwapExecutor:
    ...
    def validate_swap_parameters(self):
        ...
        try:
            ...
            print(f"✅ Preço atual: ${validation.get('current_price', 0):,.2f}")
            print(f"✅ USDT estimado: ${validation.get('estimated_usdt', 0):,.2f}")
            print(f"✅ Saldo suficiente: {validation.get('sufficient_balance')}")
            print(f"✅ Impacto no mercado: {validation.get('market_impact')}")
            print(f"✅ Estratégia recomendada: {validation.get('recommended_strategy')}")

            if not validation.get('valid'):
                print("❌ Validação falhou - swap não pode ser executado")
                return False
