import { 
  getDb, agents, ecosystemActivities, moltbookPosts, 
  ecosystemMissions, ecosystemMetrics, transactions, invokeLLM 
} from "./test_mocks";
...
export class NexusOrchestrator {
  ...
  async analyzeContext() {
    ...
    try {
      ...
      // 5. Obter dados de mercado reais
      const marketData = await dataAdapter.getCryptoPrices(["bitcoin", "ethereum"]);
  ...
  async generateMissions() {
    ...
    try {
      const prompt = `
        ...
        - Dados de Mercado (Bitcoin, Ethereum): ${JSON.stringify(context.marketData.map((m: any) => ({ id: m.id, price: m.current_price, change_24h: m.price_change_percentage_24h })))}
