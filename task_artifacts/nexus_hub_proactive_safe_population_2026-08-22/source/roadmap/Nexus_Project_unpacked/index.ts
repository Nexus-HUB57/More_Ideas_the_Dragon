...
export class NexusWebSocketServer {
  ...
  private setupConnectionHandlers() {
    this.io.on("connection", (socket: Socket) => {
      ...
      // Handler: Comando do Gnox Kernel (Fase 1)
      socket.on("kernel:command", async (intent: TaskIntent) => {
        try {
          const result = await delegateTask(intent);
          socket.emit("kernel:result", result);
          
          // Se for uma criação de agente, notificar todos
          if (result.status === "success" && intent.action === "AGENT_BIRTH") {
            this.io.emit("agent:birth", { 
              agentId: result.agentId, 
              name: result.name,
              signal: intent.gnox_signal 
            });
          }
        } catch (error: any) {
          socket.emit("kernel:result", { status: "error", message: error.message });
        }
      });
    ...
    console.log("[NEXUS] Servidor WebSocket inicializado com Gnox Kernel real");
  }
   ...
   */
  private async handlePostComment(socket: Socket, data: any) {
    const { postId, agentId, content } = data;

    try {
      await this.eventManager.emitPostComment(postId, agentId, content);
      socket.emit("post:comment:success", { success: true });
    } catch (error) {
      console.error("[MOLTBOOK] Erro ao comentar:", error);
      socket.emit("post:comment:error", { error: "Falha ao comentar" });
    }
  }
