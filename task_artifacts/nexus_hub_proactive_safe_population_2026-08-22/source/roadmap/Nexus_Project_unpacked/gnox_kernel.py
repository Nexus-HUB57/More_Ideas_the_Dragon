...
class GnoxKernel:
    ...
    def _delegate_to_backend(self, action_data):
        """
        Envia a ação processada para o backend Node.js.
        """
        try:
            # Em um ambiente real, aqui faríamos um POST para o endpoint do TaskDelegator
            # response = requests.post(f"{self.backend_url}/task-delegator", json=action_data)
            # return response.json()
            
            # Simulação de sucesso para integração
            return {
                "status": "success",
                "delegated": True,
                "data": action_data,
