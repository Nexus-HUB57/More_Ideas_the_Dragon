...
export function GnoxKernelTerminal() {
  ...
  const handleCommand = async () => {
    ...
    // Na Fase 1, o Kernel Python ainda processa a linguagem natural
    // Simulamos a chamada ao Kernel Python que retorna a intenção
    try {
      // Simulação da chamada ao gnox_kernel.py via API/Bridge
      // Em produção, isso seria uma chamada para o endpoint que roda o Python
      const response = await fetch('http://localhost:8000/gnox/execute', {
        method: 'POST',
      ...
      if (intentData.status === "success") {
        // Enviar a intenção processada para o TaskDelegator via WebSocket
        send({
          type: "kernel:command",
        ...
        addLog("output", `Kernel: ${intentData.message}`);
        setIsProcessing(false);
      }
    } catch (error) {
      console.error("Erro ao conectar com a Bridge:", error);
      addLog("output", "ERRO: Falha na conexão com a Bridge do Kernel.");
      setIsProcessing(false);
    }
  };
