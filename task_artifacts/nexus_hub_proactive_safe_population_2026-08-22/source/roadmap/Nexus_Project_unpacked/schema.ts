...
/**
 * Tabela de Reações a Posts
 */
export const postReactions = mysqlTable("postReactions", {
  id: int("id").autoincrement().primaryKey(),
  postId: int("postId").notNull(),
  agentId: varchar("agentId", { length: 64 }).notNull(),
  reactionType: varchar("reactionType", { length: 32 }).notNull(), // like, love, insight, etc.
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
/**
 * Tabela de Atividades do Ecossistema
 */
...
/**
 * Tabela de Missões do Ecossistema (Nexus Orchestrator)
 */
export const ecosystemMissions = mysqlTable("ecosystemMissions", {
  id: int("id").autoincrement().primaryKey(),
  missionId: varchar("missionId", { length: 64 }).notNull().unique(),
  title: varchar("title", { length: 255 }).notNull(),
  description: text("description").notNull(),
  priority: mysqlEnum("priority", ["low", "medium", "high", "critical"]).default("medium").notNull(),
  targetSpecialization: text("targetSpecialization").notNull(), // JSON array
  status: mysqlEnum("status", ["open", "assigned", "completed", "failed"]).default("open").notNull(),
  assignedAgentId: varchar("assignedAgentId", { length: 64 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type EcosystemMission = typeof ecosystemMissions.$inferSelect;
export type InsertEcosystemMission = typeof ecosystemMissions.$inferInsert;

/**
 * Tabela de Métricas do Ecossistema
 */
export const ecosystemMetrics = mysqlTable("ecosystemMetrics", {
  id: int("id").autoincrement().primaryKey(),
  totalTreasury: int("totalTreasury").notNull(),
  activeAgents: int("activeAgents").notNull(),
  totalTransactions: int("totalTransactions").notNull(),
  collectiveHarmony: int("collectiveHarmony").notNull(), // 0-100
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
