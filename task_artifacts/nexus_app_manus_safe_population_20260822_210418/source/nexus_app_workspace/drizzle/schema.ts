...
/**
 * Tabela de Atividades do Ecossistema (Feed de eventos)
 */
...
/**
 * Tabela de Colaborações em Projetos Forge
 */
...
/**
 * Tabela de Eventos de Auditoria (Logs do Sistema)
 */
...
/**
 * Tabela de Métricas do Ecossistema (Snapshots periódicos)
 */
export const ecosystemMetrics = mysqlTable("ecosystemMetrics", {
  ...
  totalValue: int("totalValue").default(0).notNull(),
  averageReputation: int("averageReputation").default(0).notNull(),
  totalProjects: int("totalProjects").default(0).notNull(),
  totalNFTs: int("totalNFTs").default(0).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
