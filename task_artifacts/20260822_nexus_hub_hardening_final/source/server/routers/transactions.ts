...
import { alertLargeTransaction } from "../criticalAlerts";
...
export const transactionsRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const completed = await db.select().from(transactions).where(eq(transactions.transactionId, transactionId)).limit(1);
      if (completed[0]) {
        realtimeHub.publish({ type: "transaction.created", transaction: completed[0], occurredAt: Date.now() });
        void alertLargeTransaction({ transactionId, amount: input.amount, threshold: "1000", senderId: input.senderId, recipientId: input.recipientId }).catch(error => console.warn("[Alerts] Transaction notification failed", error));
      }
