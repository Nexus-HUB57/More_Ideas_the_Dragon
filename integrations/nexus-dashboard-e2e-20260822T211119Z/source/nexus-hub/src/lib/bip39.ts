// ============================================================
// BIP39 IMPLEMENTATION — Mnemonic validation, seed derivation
...
// Source: BIP-0039 specification — public domain / BSD licensed
const WORDLIST = [
...
// ---------- ENTROPY-TO-WORD COUNT MAP ----------
const BITS_TO_WORDS: Record<number, number> = {
  128: 12, 160: 15, 192: 18, 224: 21, 256: 24,
};
const WORDS_TO_BITS: Record<number, number> = {
 ...
 * Checks: word count, all words in wordlist, checksum verification.
 */
export function validateMnemonic(mnemonic: string): MnemonicValidation {
  const words = mnemonic.trim().split(/\s+/).map(w => w.toLowerCase());

  // Check word count
  const wordCount = words.length;
  if (!WORDS_TO_BITS[wordCount]) {
    return {
      valid: false,
  ...
  // Verify checksum
  const entropyBits = WORDS_TO_BITS[wordCount];
  const cs = Math.floor(entropyBits / 32); // checksum bits
  const totalBits = entropyBits + cs;

  // Convert words back to bits
  let bits = "";
  for (const word of words) {
    const idx = WORDLIST.indexOf(word);
    const bin = idx.toString(2).padStart(11, "0");
    bits += bin;
  }

  // Split into entropy bits and checksum bits
  const entropyBin = bits.slice(0, entropyBits);
  const checksumBin = bits.slice(entropyBits);

  // Convert entropy to bytes and compute expected checksum
  const entropyBytes = Buffer.from(
    entropyBin.match(/.{1,8}/g)!.map(b => parseInt(b, 2))
  );
  const hash = sha256(entropyBytes);
  const hashBin = hash.toString("hex").split("").map(c =>
    parseInt(c, 16).toString(2).padStart(4, "0")
  ).join("");
  const expectedChecksum = hashBin.slice(0, cs);

  if (checksumBin !== expectedChecksum) {
    return {
      valid: false,
      error: "Checksum mismatch — mnemonic is invalid or corrupted.",
      wordCount,
      entropyBits,
    };
  }

  return { valid: true, wordCount, entropyBits };
}

/**
 * Convert a BIP39 mnemonic to a 512-bit seed using PBKDF2-SHA512.
 * passphrase defaults to empty string (BIP39 standard).
 */
 ...
 * @param strength - Entropy in bits: 128 (12 words), 160 (15), 192 (18), 224 (21), 256 (24)
 */
export function generateMnemonic(strength: number = 128): string {
  if (!BITS_TO_WORDS[strength]) {
    throw new Error(`Invalid entropy strength: ${strength}. Use 128, 160, 192, 224, or 256.`);
  }

  const entropy = crypto.randomBytes(strength / 8);
  const hash = sha256(entropy);
  const hashBits = hash.toString("hex").split("").map(c =>
    parseInt(c, 16).toString(2).padStart(4, "0")
  ).join("");

  const csBits = Math.floor(strength / 32);
  const totalBits = strength + csBits;
  ...
  const allBits = entropyBits + hashBits.slice(0, csBits);
