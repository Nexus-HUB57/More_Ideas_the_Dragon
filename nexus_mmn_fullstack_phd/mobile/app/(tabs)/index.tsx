...
export default function HomeScreen() {
  ...
  } = trpc.dashboard.getMetrics.useQuery(undefined, { refetchInterval: 5000 });
  const {
    data: recentSalesData,
    isLoading: isLoadingRecentSales,
    refetch: refetchRecentSales,
  } = trpc.dashboard.getRecentSales.useQuery(
    { limit: 3 },
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    await Promise.all([refetchMetrics(), refetchRecentSales()]);
    setRefreshing(false);
  };
