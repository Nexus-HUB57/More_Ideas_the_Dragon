...
describe("AI Content Hub - Integration Tests", () => {
  ...
  describe("Post Scheduling Flow", () => {
    ...
    it("should schedule posts at different times", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const result = await caller.schedulePost({
          content: `Post content for agendamento ${i + 1}`,
          platforms: ["instagram"],
          scheduledFor,
        });
    ...
    it("should handle invalid model gracefully", async () => {
      try {
        await caller.generateContent({
          prompt: "Generate content for testing",
      ...
      const requests = Array(5)
        ...
        .map((_, i) =>
          caller.generateContent({
            prompt: `Generate content for testing ${i}`,
            modelId: "gemini-2.0-flash",
          })
        );
  ...
  describe("Data Consistency", () => {
    it("should maintain consistency across multiple operations", async () => {
      // Generate content
      const contentResult = await caller.generateContent({
        prompt: "Create marketing content for testing",
        modelId: "gemini-2.0-flash",
      });
  ...
  describe("Performance Metrics", () => {
    it("should generate content within acceptable time", async () => {
      ...
      const result = await caller.generateContent({
        prompt: "Generate marketing content for testing",
        modelId: "gemini-2.0-flash",
      });
    ...
    it("should handle batch operations efficiently", async () => {
      ...
      const results = await Promise.all([
        caller.generateContent({
          prompt: "Content 1 for testing",
          modelId: "gemini-2.0-flash",
        }),
        caller.generateContent({
          prompt: "Content 2 for testing",
          modelId: "gemini-2.0-flash",
        }),
        caller.generateContent({
          prompt: "Content 3 for testing",
          modelId: "gemini-2.0-flash",
        }),
      ]);
