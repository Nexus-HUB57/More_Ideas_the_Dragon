...
import { calculateUnilevelCommission, getUnilevelCommissionRate } from './commissionRules';
...
export async function calculateUnilevelCommissions(
  ...
  const rate = getUnilevelCommissionRate(level);
  if (rate === 0) return;

  const commissionAmount = calculateUnilevelCommission(amount, level);
