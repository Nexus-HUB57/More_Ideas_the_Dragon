...
export class NexusWebSocketServer {
  ...
  broadcastHarmonyChange(harmonyLevel: number) {
    this.io.emit("harmony:change", { harmonyLevel });
  }

  broadcastAgentStatus(agentData: any) {
    this.io.to("metrics").emit("agents:status", agentData);
  }
