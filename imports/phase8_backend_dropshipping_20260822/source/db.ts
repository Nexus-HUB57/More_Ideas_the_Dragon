...
import { InsertUser, users, agents, upgrades, agentUpgrades, InsertAgent, Agent, Upgrade, AgentUpgrade } from "./schema-final";
import { ENV } from './env';
...
export async function createNotification(data: {
  ...
  try {
    const { notifications } = await import("./schema-final");
...
export async function getAffiliateByUserId(userId: number) {
  ...
  const { affiliates } = await import("./schema-final");
...
export async function getDirectReferrals(userId: number) {
  const db = await getDb();
  if (!db) return [];
  const { network } = await import("./schema-final");
...
export async function getOrdersByAffiliate(affiliateId: number, limit: number = 50) {
  const db = await getDb();
  if (!db) return [];
  const { orders } = await import("./schema-final");
...
export async function getTrendingProducts(limit: number = 10) {
  const db = await getDb();
  if (!db) return [];
  const { products } = await import("./schema-final");
