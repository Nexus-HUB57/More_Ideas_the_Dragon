const int = (n: string) => ({ autoincrement: () => ({ primaryKey: () => ({}) }), primaryKey: () => ({}) });
const mysqlEnum = (n: string, choices: string[]) => ({ default: () => ({ notNull: () => ({}) }), notNull: () => ({}) });
const mysqlTable = (n: string, cols: any) => ({ $inferSelect: {}, $inferInsert: {} });
const text = (n: string) => ({});
const timestamp = (n: string) => ({ defaultNow: () => ({ notNull: () => ({ onUpdateNow: () => ({ notNull: () => ({}) }) }), onUpdateNow: () => ({ notNull: () => ({}) }) }) });
const varchar = (n: string, opts: any) => ({ notNull: () => ({ unique: () => ({}) }), unique: () => ({}) });
