...
# Chave privada para assinatura (WIF)
PRIVATE_KEY_WIF = '<YOUR_PRIVATE_KEY_WIF>'

# Configurações do nó Bitcoin (RPC)
# Para este ambiente de sandbox, não temos um nó Bitcoin completo.
# Vamos simular a interação com o RPC para construir a transação.
# Em um ambiente real, você conectaria a um nó Bitcoin local.
...
def main():
    ...
    print("Iniciando geração e assinatura da primeira transação (P2PKH para P2PKH intermediário) usando python-bitcoinrpc...")
    ...
    try:
        ...
        # Construir a transação raw (sem assinatura)
        # Usamos um dicionário para representar a transação raw, que seria então passada para 'createrawtransaction'
        # e 'signrawtransactionwithkey' de um nó Bitcoin via RPC.
        ...
        # Outputs
        outputs = {
            DESTINATION_ADDRESS_INTERMEDIATE: AMOUNT_SATOSHIS / 100_000_000.0 # Converter para BTC
        }
        if change_value > 546: # Dust limit para P2PKH
            outputs[source_address] = change_value / 100_000_000.0 # Converter para BTC

        # Simular a criação da transação raw (em um ambiente real, isso seria feito via RPC)
        # Exemplo de como seria a chamada RPC:
        # raw_tx_unsigned = rpc_connection.createrawtransaction(inputs, outputs)
        # print(f"Transação raw não assinada: {raw_tx_unsigned}")

        # Simular a assinatura da transação raw (em um ambiente real, isso seria feito via RPC)
        # Exemplo de como seria a chamada RPC:
        # signed_tx_result = rpc_connection.signrawtransactionwithkey(raw_tx_unsigned, [PRIVATE_KEY_WIF])
        # tx_hex = signed_tx_result["hex"]

        # Como não temos um nó RPC, vamos gerar um placeholder para o hex da transação
        tx_hex = "PLACEHOLDER_RAW_TRANSACTION_HEX_FROM_BITCOINRPC"
        ...
        output_data = {
            ...
            "status": "signed_with_python_bitcoinrpc_simulated",
            "used_utxos": [selected_utxo]
        }

        output_filename = f'signed_transaction_bitcoinrpc_1_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        with open(output_filename, "w") as f:
            json.dump(output_data, f, indent=2)
        print(f"\nPrimeira transação assinada com sucesso (simulada) e salva em {output_filename}")
        print("Raw Transaction Hex (simulada com python-bitcoinrpc):\n" + tx_hex)
